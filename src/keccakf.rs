...
pub fn round_64bits(a: [u64; WIDTH_IN_WORDS], round: usize) -> [u64; WIDTH_IN_WORDS] {
    let mut a = a;
    // θ-Theta step
    ...
    // ι-Iota step
    // A[0,0] = A[0,0] xor RC
    let rc_u64 = crate::params::RC_64_BITS[round];

    a[0] = xor(a[0], rc_u64);
    return a;
}

pub fn keccakf_64bits(input: [u64; WIDTH_IN_WORDS]) -> [u64; WIDTH_IN_WORDS] {
    let mut a = input;
...
#[cfg(test)]
mod test {
    use crate::keccakf::round_64bits;

    #[test]
    fn test_permutation_test() {
        let inputs = [4_u64; 25];
        let output = round_64bits(inputs, 1);
